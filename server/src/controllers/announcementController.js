...
async function accessFilter(user) {
...
export const listAnnouncements = asyncHandler(async (req, res) => {
  const f = await accessFilter(req.user);
  const data = await Announcement.find(f)
...
export const getAnnouncement = asyncHandler(async (req, res) => {
  ...
  if (req.user.role === "STUDENT") {
    ...
    if (
      !a.isPublished ||
      (a.targetType === "BATCH" &&
        !a.targetBatches.map(String).includes(String(s?.batchId)))
    )
...
export const createAnnouncement = asyncHandler(async (req, res) => {
  ...
  await logActivity(req, {
    ...
    description: `Created announcement ${a.title}`,
    entityType: "Announcement",
    entityId: a._id,
  });
  if (a.isPublished) {
    const ids = await targetStudentUserIds(a.targetType, a.targetBatches);
  ...
  res
    .status(201)
    .json({ success: true, message: "Announcement created", data: a });
});
export const updateAnnouncement = asyncHandler(async (req, res) => {
  ...
  await logActivity(req, {
    ...
    module: "ANNOUNCEMENTS",
    description: `Updated announcement ${a.title}`,
    entityType: "Announcement",
    entityId: a._id,
  });
  res.json({ success: true, message: "Announcement updated", data: a });
});
export const deleteAnnouncement = asyncHandler(async (req, res) => {
  ...
  await logActivity(req, {
    ...
    module: "ANNOUNCEMENTS",
    description: `Deleted announcement ${a.title}`,
    entityType: "Announcement",
    entityId: a._id,
  });
...
export const publishAnnouncement = asyncHandler(async (req, res) => {
  ...
  if (!a)
    throw new ApiError(404, "Announcement not found", "ANNOUNCEMENT_NOT_FOUND");
  res.json({ success: true, message: "Publication status updated", data: a });
});
